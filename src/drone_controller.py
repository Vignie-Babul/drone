...
class DroneController:
    ...
    def update(self, dt):
        ...
        acceleration = self.config.get('acceleration', 5.0)
        rotation_speed = self.config.get('rotation_speed', 90.0)
        ...
        if parent:
            up_vec = parent.getRelativeVector(self.tilt_node, Vec3(0, 0, 1))
        else:
            up_vec = Vec3(0, 0, 1)
