...
class Analytics:
	...
	def _send_to_server(self, analytics):
		print(analytics, file=sys.stderr)

	def _dump_to_file(self, analytics):
		with open(self._path, 'a', encoding='utf-8') as file:
			json.dump(analytics, file, ensure_ascii=False, indent=2)
			file.write('\n')

	def send(self, analytics, imitate=None):
...
class DataSave:
	...
	def dump(self, best_time=0.0, total_score=0, unlocked_levels=0, avg_speed=0.0, max_speed=0.0, drone_config=None):
